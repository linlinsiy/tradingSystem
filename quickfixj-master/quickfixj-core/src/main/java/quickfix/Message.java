...
public class Message extends FieldMap {
        ...
        while (field != null) {
            ...
            if (isHeaderField(field.getField())) {
                ...
                if (doValidation && dd != null && dd.isCheckFieldsOutOfOrder())
                    throw new FieldException(SessionRejectReason.ORDERED_TAG_ORDER_ERROR,
                        field.getTag());
            } else {
                setField(this, field);
                // Group case
    ...
    private void setField(FieldMap fields, StringField field) {
        if (fields.isSetField(field)) {
            throw new FieldException(SessionRejectReason.SEVERAL_TIMES_IN_SAME_TAG, field.getTag());
        }
        fields.setField(field);
    }
                    ...
                    if (offset > -1) {
                        if (offset <= previousOffset) {
                            ...
                            throw new FieldException(
                                    SessionRejectReason.REPEAT_TAG_ORDER_ERROR, tag);
                        }
    ...
    private void checkFirstFieldFound(boolean firstFieldFound, final int groupCountTag, final int firstField, int tag) throws FieldException {
        if (!firstFieldFound) {
            throw new FieldException(
                    SessionRejectReason.REPEAT_TAG_ORDER_ERROR, "The group " + groupCountTag
